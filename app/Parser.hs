...
noteTitle = T.pack <$> try (optional (((digitChar *> char '.') <|> char '*') *> sc) *> optional (symbol "Historic") *> symbol "Note" *> (symbol "[" *> manyTill anySingle (char ']')) <* sc <* newline <* sc <* some (char '~') <* newline)
